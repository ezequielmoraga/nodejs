...
const servidor = http.createServer((req,res)=>{
    ...
    switch(method)
    {
        ...
        case 'DELETE':
          return manejarSolicitudDELETE(req,res);// AGREGE ESTE DELETE
...
function manejarSolicitudGET(req,res)
{
        ...
        {
            console.log('Se recibió GET /api/Vehiculos');
...
function manejarSolicitudDELETE(req,res){
  const path = req.url;                          // /api/Choferes/12223333
  if (path.startsWith('/api/Choferes/')) {
    const partes = path.split('/');
    const dni = partes[3] || '';
    console.log(`Se recibió DELETE para /api/Choferes/${dni}`);
    res.writeHead(200, {'Content-Type':'text/plain; charset=utf-8'});
    return res.end(`Solicitud DELETE recibida para DNI: ${dni}`);
  } else {
    res.statusCode = 404;
    return res.end('No existe el recurso');
  }
